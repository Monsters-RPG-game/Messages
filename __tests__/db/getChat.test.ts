...
describe('Chat - get', () => {
  ...
  describe('Should pass', () => {
    it(`Get all`, async () => {
      ...
      const messages = await repository.getByOwner(fakeMessage.sender, 1);
    ...
    it(`Get one`, async () => {
      ...
      const message = await repository.getOne(fakeMessage.sender, fakeMessage.receiver);
    ...
    it(`Get unread`, async () => {
      ...
      const messages = await repository.getUnread(fakeMessage.sender, 1);
      const message = messages[0]!;
    ...
    it(`Get with details`, async () => {
      ...
      const messages = await repository.getWithDetails(fakeMessage.chatId, 1);
      const message = messages[0]!;
